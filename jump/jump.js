(() => {
  ...
  function update() {
    ...
    // Keep player in bounds (bounce off walls)
    if (player.x < 0) { player.x = 0; if (player.vx < 0) player.vx *= -0.4; }
    if (player.x + player.w > canvas.width) { player.x = canvas.width - player.w; if (player.vx > 0) player.vx *= -0.4; }
